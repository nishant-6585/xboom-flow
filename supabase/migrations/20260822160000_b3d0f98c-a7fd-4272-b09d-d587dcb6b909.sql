...
-- ------------------------------------------------------------- 3. backfill
-- Existing rows carry no reason. Rather than guess, mark as earned only what
-- can be proven earned: the owner is also the agent who answered, resolved
-- through agent_user_mapping — the same path the webhook uses. Everything else
-- stays NULL and is treated as provisional, so the next rep to actually answer
-- takes the number over. That is the intended correction, not a side effect:
-- those owners were assigned at random and never spoke to the caller.
DO $backfill$
DECLARE v_rows INT;
BEGIN
