DROP FUNCTION IF EXISTS public.get_next_birthday();

CREATE FUNCTION public.get_next_birthday()
RETURNS TABLE (
  employee_id uuid,
  name text,
  department text,
...
BEGIN
  ...
  IF NOT public.is_user_approved(v_uid) THEN RAISE EXCEPTION 'Forbidden'; END IF;

  SELECT EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = v_uid AND ur.role <> 'b2b_customer'::app_role
  ) INTO v_has_staff;
  IF NOT v_has_staff THEN RAISE EXCEPTION 'Forbidden'; END IF;
  ...
  SELECT
    c.employee_id, c.name, c.department, c.avatar_url, c.bm, c.bd,
