-- ============================================================================
-- Walk-in leads — customers who turn up in person
-- ============================================================================
-- A walk-in is a lead a salesperson creates while standing in front of the
-- customer. That changes what the record needs, in ways the existing web-form
-- lead shape does not cover:
--
--   * It is already "contacted" the moment it exists — the conversation is
--     happening. Creating it as untouched would be false, and would put it in
--     chase-up reports it does not belong in.
--   * It belongs to whoever took the walk-in. There is no round-robin to run.
--   * The useful facts are about the VISIT: which showroom, what they looked
--     at, what happened, when to follow up. None of that fits a form payload.
--
-- Rather than a parallel leads table, walk-ins are rows in public.leads with
-- source = 'walk_in', so they flow into everything that already reads leads —
-- the unified feed, dispositions, company rollups. Visit specifics live in a
-- 1:1 companion table so they stay queryable ("walk-ins per store this
-- month") instead of being buried in jsonb.
-- ============================================================================
...
COMMENT ON TABLE public.lead_walk_in_details IS
  'Visit detail for leads with source = ''walk_in''. 1:1 with public.leads.';

ALTER TABLE public.lead_walk_in_details
  DROP CONSTRAINT IF EXISTS lead_walk_in_timeline_check;
ALTER TABLE public.lead_walk_in_details
  ADD CONSTRAINT lead_walk_in_timeline_check CHECK (
    purchase_timeline IS NULL OR purchase_timeline IN
      ('immediate', 'this_week', 'this_month', 'this_quarter', 'exploring')
  );

ALTER TABLE public.lead_walk_in_details
  DROP CONSTRAINT IF EXISTS lead_walk_in_outcome_check;
ALTER TABLE public.lead_walk_in_details
  ADD CONSTRAINT lead_walk_in_outcome_check CHECK (
    visit_outcome IS NULL OR visit_outcome IN
      ('purchased', 'quote_requested', 'demo_given', 'will_return',
       'just_browsing', 'not_interested')
  );

CREATE INDEX IF NOT EXISTS idx_walk_in_visited_at
  ON public.lead_walk_in_details (visited_at DESC);
CREATE INDEX IF NOT EXISTS idx_walk_in_follow_up
  ON public.lead_walk_in_details (follow_up_at)
  WHERE follow_up_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_walk_in_store
  ON public.lead_walk_in_details (store_location);
CREATE INDEX IF NOT EXISTS idx_leads_walk_in
  ON public.leads (created_at DESC) WHERE source = 'walk_in';

ALTER TABLE public.lead_walk_in_details ENABLE ROW LEVEL SECURITY;

-- Visibility mirrors how the team already works: a rep sees their own, and
-- managers/admins see everything. Deliberately NOT "any sales sees all" —
-- that is the existing convention for lead ownership.
...
CREATE POLICY "walk_in: read own or manage"
  ...
  USING (
    ...
    OR EXISTS (SELECT 1 FROM public.leads l
                WHERE l.id = lead_walk_in_details.lead_id
...
-- ---------------------------------------------------------------------------
-- Auto-assign to whoever took the walk-in.
--
-- A trigger rather than client-side, so it holds no matter which surface
-- creates the row, and cannot be forgotten by a future writer.
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  v_name text;
BEGIN
  IF NEW.source IS DISTINCT FROM 'walk_in' THEN
    RETURN NEW;
  END IF;
  ...
  IF NEW.assigned_to IS NOT NULL
     AND (NEW.assigned_to_name IS NULL OR NEW.assigned_to_name = '') THEN
    SELECT COALESCE(NULLIF(p.name, ''), p.email) INTO v_name
      FROM public.profiles p WHERE p.user_id = NEW.assigned_to;
    NEW.assigned_to_name := v_name;
  END IF;

  -- The salesperson is talking to them right now. Recording this as untouched
  -- would be a lie, and would feed follow-up-chasing reports with leads that
  -- have already been spoken to.
  IF NEW.last_contacted_at IS NULL THEN
    NEW.last_contacted_at := now();
  END IF;
  IF NEW.disposition = 'untouched'::lead_disposition THEN
    NEW.disposition := 'prospect'::lead_disposition;
  END IF;
  IF NEW.status IS NULL OR NEW.status = 'new' THEN
    NEW.status := 'contacted';
  END IF;
  IF NEW.form_type IS NULL THEN
    NEW.form_type := 'walk-in';
  END IF;
...
CREATE TRIGGER trg_leads_walk_in_defaults
  ...
  FOR EACH ROW EXECUTE FUNCTION public.leads_set_walk_in_defaults();

-- ---------------------------------------------------------------------------
-- Create a walk-in in one call, so the lead and its visit detail cannot end up
-- half-written if the client dies between two inserts.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT (has_role(v_uid, 'sales'::app_role)
       OR has_role(v_uid, 'sales_manager'::app_role)
       OR has_role(v_uid, 'admin'::app_role)) THEN
  ...
  INSERT INTO public.leads
    (source, form_type, name, phone, email, company, subject, message,
     assigned_to, assigned_to_name, status, disposition, last_contacted_at,
  ...
  INSERT INTO public.lead_walk_in_details
    (lead_id, visited_at, store_location, accompanied_by, products_interested,
     budget_range, purchase_timeline, visit_outcome, follow_up_at,
     referral_source, notes, created_by, created_by_name)
  VALUES
...
-- ---------------------------------------------------------------------------
-- Duplicate check. Walk-ins are the most likely place to re-enter someone who
-- is already in the pipeline — they phoned last month, now they are at the
-- counter. Surfacing that BEFORE the row is written keeps the pipeline honest
-- and stops two reps unknowingly working the same customer.
--
-- Matches on the last 10 digits of the phone, so +91/0/spacing variants still
...
BEGIN
  ...
  RETURN QUERY
  SELECT f.source, f.source_row_id, f.name, f.phone, f.email, f.company,
         f.status, f.sales_person_name, f.created_at
    FROM public.unified_lead_feed f
   WHERE (v_digits IS NOT NULL
          AND right(regexp_replace(COALESCE(f.phone, ''), '\D', '', 'g'), 10) = v_digits)
...
-- ---------------------------------------------------------------------------
-- The walk-in list, joined to its visit detail.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.list_walk_in_leads(
...
BEGIN
  IF v_uid IS NULL THEN RETURN; END IF;
  v_manages := has_role(v_uid, 'admin'::app_role)
            OR has_role(v_uid, 'sales_manager'::app_role)
            OR has_role(v_uid, 'supply_chain'::app_role);
  IF NOT (v_manages OR has_role(v_uid, 'sales'::app_role)) THEN RETURN; END IF;

  RETURN QUERY
  SELECT l.id, l.name, l.phone, l.email, l.company, l.status,
         l.disposition::text, l.assigned_to, l.assigned_to_name, l.created_at,
         d.visited_at, d.store_location, d.products_interested, d.budget_range,
         d.purchase_timeline, d.visit_outcome, d.follow_up_at,
         d.referral_source, d.accompanied_by, d.notes,
         (d.follow_up_at IS NOT NULL
          AND d.follow_up_at < now()
          AND COALESCE(d.visit_outcome, '') NOT IN ('purchased', 'not_interested')
...
-- ---------------------------------------------------------------------------
-- Update the visit outcome / follow-up after the fact.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.update_walk_in_outcome(
  _lead_id       bigint,
  _visit_outcome text DEFAULT NULL,
  _follow_up_at  timestamptz DEFAULT NULL,
  _notes         text DEFAULT NULL
...
BEGIN
  ...
  -- A recorded outcome is a contact. Keep the lead's own timeline honest.
  UPDATE public.leads
     SET last_contacted_at = now(),
         status = CASE WHEN _visit_outcome = 'purchased' THEN 'converted'
                       WHEN _visit_outcome = 'not_interested' THEN 'archived'
                       ELSE status END,
         disposition = CASE WHEN _visit_outcome = 'not_interested'
                            THEN 'not_qualified'::lead_disposition
                            WHEN _visit_outcome = 'purchased'
                            THEN 'qualified'::lead_disposition
