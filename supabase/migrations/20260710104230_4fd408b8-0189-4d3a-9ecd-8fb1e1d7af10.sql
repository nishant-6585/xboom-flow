...
RETURNS TABLE(
  id uuid, order_number text, source text, external_id text, sales_person_name text,
  total_sales_amount numeric, order_date date, created_at timestamp with time zone,
  customer_name text, customer_phone text, product_name text,
  amount_diff_pct numeric, match_reasons text[]
)
LANGUAGE plpgsql
...
DECLARE
  v_uid uuid := auth.uid();
BEGIN
  -- Service-role / trigger callers (no JWT) remain allowed.
