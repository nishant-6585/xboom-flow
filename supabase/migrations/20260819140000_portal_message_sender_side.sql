...
-- ============================================================================
-- Two rounds of bugs have come from guessing whether a message is from the
-- customer or from staff:
--
--   1. Guessing by ROLE broke for a staff member who is also a portal contact:
--      their customer replies were read as staff replies, so the team was
--      never alerted (fixed in 20260819100000 by switching to identity).
--   2. Guessing by IDENTITY breaks when the SAME auth user is both the contact
--      and the replying staff member. The heuristic answers "customer" for
--      every message they send, so their admin-side replies never notify the
--      customer.
--
-- No property of the sender can resolve case 2, because there is nothing to
-- tell apart — it is one person. The distinguishing fact is WHICH SURFACE the
-- message was written in, and only the client knows that. So record it.
--
-- sender_side is set explicitly by each writer:
--   portal reply / new ticket      -> 'customer'
--   admin reply / quick reply      -> 'staff'
--   inbound WhatsApp webhook       -> 'customer'
--
-- When a writer omits it, a BEFORE INSERT trigger falls back to the identity
-- heuristic, so older clients and any writer added later keep working.
-- ============================================================================
...
-- Backfill history with the best available guess.
...
-- ---------------------------------------------------------------------------
-- Default it when a writer omits it.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Staff alerting: customer messages only.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Staff replying to the customer: the customer is told by the sibling
  -- trigger and by portal-notify. Nothing for staff to hear about.
  ...
  -- description. That message is the ticket itself, not a reply — without this
  -- guard every new ticket would alert twice on every channel.
  IF NOT EXISTS (
...
-- ---------------------------------------------------------------------------
-- Customer alerting: staff messages only.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NEW.sender_side IS DISTINCT FROM 'staff' THEN
    RETURN NEW;  -- a customer's own message must not notify them
  END IF;
...
-- ---------------------------------------------------------------------------
-- Inbox unread count: same authoritative column, so the badge and the alerts
-- agree by construction rather than by two copies of a heuristic.
-- ---------------------------------------------------------------------------
