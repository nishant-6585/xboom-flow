...
-- 1) Add evidence columns (jsonb array of {kind, id, label, url?})
...
-- 2) Extend _attribute_website_order_core to persist evidence into the log
...
-- 3) Update attribute_website_order to accept p_evidence and forward it
...
GRANT EXECUTE ON FUNCTION public.attribute_website_order(uuid, uuid, text, text, jsonb) TO authenticated;

-- 4) Update request_website_order_attribution to accept and store p_evidence
...
GRANT EXECUTE ON FUNCTION public.request_website_order_attribution(uuid, text, text, jsonb) TO authenticated;
