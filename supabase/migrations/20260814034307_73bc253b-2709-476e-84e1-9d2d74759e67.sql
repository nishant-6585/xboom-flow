...
BEGIN
  ...
  v_is_manager := public.has_role(auth.uid(), 'admin')
               OR public.has_role(auth.uid(), 'sales_manager');

  -- Reps can only ever see their own numbers.
  ...
  WITH
  -- ============ LEAD UNIVERSE (one row per lead, one owner) ============
  lead_rows AS (
...
REVOKE ALL ON FUNCTION public.get_sales_dashboard_metrics(date, date, uuid, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_sales_dashboard_metrics(date, date, uuid, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_sales_dashboard_metrics(date, date, uuid, boolean) TO service_role;