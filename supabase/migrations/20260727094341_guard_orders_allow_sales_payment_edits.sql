-- Allow sales role to update amount_paid / payment_status on their OWN orders
-- (RLS enforces ownership). Total sales amount, selling_price, refund fields
-- and sales_person_id remain reserved for admin/finance/sales_manager/supply_chain.
...
BEGIN
  ...
  is_sales := public.has_role(auth.uid(), 'sales');
  ...
  -- Fields still fully reserved for privileged roles
  ...
  THEN
    RAISE EXCEPTION 'Only admin/finance/sales_manager/supply_chain can change selling price, refund, or attribution fields on orders'
      USING ERRCODE = '42501';
  END IF;

  -- amount_paid / payment_status can now be edited by the sales owner
  IF (NEW.payment_status IS DISTINCT FROM OLD.payment_status
      OR NEW.amount_paid IS DISTINCT FROM OLD.amount_paid)
  THEN
    IF NOT (is_sales AND is_own) THEN
      RAISE EXCEPTION 'Only the sales owner (or admin/finance/sales_manager/supply_chain) can change payment status or amount paid'
        USING ERRCODE = '42501';
    END IF;
  END IF;
