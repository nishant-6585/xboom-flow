...
BEGIN
  ...
  -- Pick the primary product line, excluding shipping/courier lines
  SELECT product_name, product_category, quantity
    INTO v_first
    FROM public.order_items
   WHERE order_id = v_order_id
     AND COALESCE(product_category, '') <> 'Shipping'
     AND COALESCE(product_code, '') NOT IN ('flat_rate', 'free_shipping', 'local_pickup', 'shipping')
   ORDER BY created_at ASC, id ASC
   LIMIT 1;

  -- Fallback: if only shipping lines exist, use whatever is first
  IF NOT FOUND THEN
    SELECT product_name, product_category, quantity
      INTO v_first
      FROM public.order_items
  ...
  -- Count only non-shipping items for the "+N more" suffix
...
-- Backfill: recompute titles for all existing orders using the new logic
WITH primary_item AS (
  ...
  WHERE COALESCE(oi.product_category, '') <> 'Shipping'
    AND COALESCE(oi.product_code, '') NOT IN ('flat_rate', 'free_shipping', 'local_pickup', 'shipping')
  ORDER BY oi.order_id, oi.created_at ASC, oi.id ASC
),
counts AS (
  SELECT order_id, COUNT(*) AS c
    FROM public.order_items
   WHERE COALESCE(product_category, '') <> 'Shipping'
     AND COALESCE(product_code, '') NOT IN ('flat_rate', 'free_shipping', 'local_pickup', 'shipping')
   GROUP BY order_id
)
