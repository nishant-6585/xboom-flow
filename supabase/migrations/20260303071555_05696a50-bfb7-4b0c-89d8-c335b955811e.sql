...
-- Update can_view_hr_folder to also check user_roles when matching department shares
    ...
    OR EXISTS (
      ...
      AND (
        ...
        OR (fs.share_type = 'department' AND (
          -- Match against employee department
          fs.department IN (SELECT department FROM public.employees WHERE user_id = _user_id)
          -- Also match against user roles (HR, Admin, etc. are roles not departments)
...
-- Similarly update can_view_hr_document
...
AS $$
  SELECT 
    -- Uploader can always see their own document
    EXISTS (
      SELECT 1 FROM public.hr_documents d
      WHERE d.id = _document_id AND d.uploaded_by = _user_id
    )
    -- Document in a folder the user can view
    OR EXISTS (
      SELECT 1 FROM public.hr_documents d
      WHERE d.id = _document_id
      AND public.can_view_hr_folder(_user_id, d.folder_id)
    )
    ...
    OR EXISTS (
      ...
      AND (
        ...
        OR (ds.share_type = 'department' AND (
          ds.department IN (SELECT department FROM public.employees WHERE user_id = _user_id)
          OR LOWER(ds.department) IN (SELECT role::text FROM public.user_roles WHERE user_id = _user_id)
        ))
      )
    )
$$;
