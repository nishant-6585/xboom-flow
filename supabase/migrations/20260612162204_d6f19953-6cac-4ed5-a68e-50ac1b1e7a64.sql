...
BEGIN
  ...
  v_phone_norm := NULLIF(RIGHT(REGEXP_REPLACE(COALESCE(v_phone, ''), '\D', '', 'g'), 10), '');
  ...
  IF v_phone_norm IS NOT NULL OR v_email_norm IS NOT NULL THEN
    ...
    IF v_dup.sales_person_id IS NOT NULL THEN
      v_sales_person_id := v_dup.sales_person_id;
      v_sales_person_name := v_dup.sales_person_name;
    END IF;
  END IF;
...
WITH normalized AS (
  SELECT
    ...
    sales_person_name,
    created_at,
    NULLIF(RIGHT(REGEXP_REPLACE(COALESCE(phone, ''), '\D', '', 'g'), 10), '') AS phone_norm,
    NULLIF(LOWER(TRIM(COALESCE(email, ''))), '') AS email_norm
  FROM public.form_leads
),
phone_primary AS (
  SELECT DISTINCT ON (phone_norm) phone_norm AS key, sales_person_id, sales_person_name
  FROM normalized
  WHERE phone_norm IS NOT NULL AND sales_person_id IS NOT NULL
  ORDER BY phone_norm, created_at ASC
),
email_primary AS (
  SELECT DISTINCT ON (email_norm) email_norm AS key, sales_person_id, sales_person_name
  FROM normalized
  WHERE email_norm IS NOT NULL AND sales_person_id IS NOT NULL
  ORDER BY email_norm, created_at ASC
),
resolved AS (
  SELECT
    n.id,
    COALESCE(pp.sales_person_id, ep.sales_person_id) AS new_sp_id,
    COALESCE(pp.sales_person_name, ep.sales_person_name) AS new_sp_name
  FROM normalized n
  LEFT JOIN phone_primary pp ON pp.key = n.phone_norm
  LEFT JOIN email_primary ep ON ep.key = n.email_norm
)
UPDATE public.form_leads fl
SET sales_person_id = r.new_sp_id,
    sales_person_name = r.new_sp_name,
    updated_at = now()
FROM resolved r
WHERE fl.id = r.id
  AND r.new_sp_id IS NOT NULL
  AND fl.sales_person_id IS DISTINCT FROM r.new_sp_id;