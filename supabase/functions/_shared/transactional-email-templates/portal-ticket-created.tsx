...
const Email = (p: Props = {}) => {
  const subj = `[${(p.priority || '').toUpperCase()}] Ticket ${p.ticket_number || ''}: ${p.subject || ''}`
  return (
      ...
      <Body style={main}>
        <Container style={container}>
          <Section style={card}>
            ...
            <Text style={t}>New ticket from <b>{p.company_name || '—'}</b>.</Text>
            {p.ticket_url ? (
...
export const template = {
  component: Email,
  subject: (d: Record<string, any>) => `[${(d?.priority || '').toUpperCase()}] Ticket ${d?.ticket_number || ''}: ${d?.subject || ''}`,
  displayName: 'Portal — Ticket Created (internal)',
  transactional: true,
  previewData: {
