...
const Email = (p: Props = {}) => {
  const subj = `Reply on ${p.ticket_number || ''} — ${p.subject || ''}`
  return (
      ...
      <Body style={main}>
        <Container style={container}>
          <Section style={card}>
            ...
            <Text style={t}><b>{p.sender_name || 'Customer'}</b> replied:</Text>
...
export const template = {
  component: Email,
  subject: (d: Record<string, any>) => `Reply on ${d?.ticket_number || ''} — ${d?.subject || ''}`,
  displayName: 'Portal — Ticket Customer Reply (staff)',
  transactional: true,
  previewData: {
    ticket_number: 'PT-3001',
    subject: 'Cannot download invoice',
    sender_name: 'Jane Doe',
    body: 'Still failing on Chrome 126.',
