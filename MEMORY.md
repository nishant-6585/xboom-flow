...
- **KYC follow-ups (#2/#3/#4) 🟡 in progress (prompt given to Lovable):** admin view to browse/filter `kyc_email_log`; pure-helper refactor + vitest tests for retry/idempotency/attempt_count; RLS confirm + tighten `feature_flags` SELECT to admin-only.
