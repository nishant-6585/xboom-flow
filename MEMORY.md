...
- **Hardening Part 2 ⏳ STILL PENDING:** non-consuming invite link — `portal_invite_tokens` table + `/portal/activate` page + `portal-activate` edge fn, replacing the embedded one-time recovery token (which scanners/previews can consume). Mirror in manual `portal-invite-customer`. Not yet built.
