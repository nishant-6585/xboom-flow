...
- **Confirmation sends — in flight:** ⏳ DLT template `OrderConfirmationRequest` (static URL, XBOOMT header) awaiting approval → then MSG91 flow ID to Lovable. ⏳ Email consolidation prompt (single onboarding email from notifications@xboom.in with password+KYC+confirm steps; standalone confirm email only for existing portal customers; /portal/confirm gated on KYC submitted) — handed over, not yet confirmed built. Root cause of missing confirm email: sender `orders@xboomflow.com` unverified in Resend.
