...
const statCards = (counts: Record<string, number>) => [
  { label: "Total Candidates", value: counts.total, icon: Users, color: "text-blue-500" },
...
export default function Candidates() {
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 text-center">
          ...
          <p className="text-muted-foreground">The Candidates module is only accessible to Admin and HR roles.</p>
        </main>
        <MobileBottomNav />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <main className="container mx-auto px-4 py-6 pb-24 sm:pb-8 space-y-6">
        {/* Page Header */}
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold">Candidate Database</h1>
            <p className="text-muted-foreground text-sm mt-0.5">Manage all candidates — selected, rejected, and archived</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {statCards(counts).map((card) => (
            <div key={card.label} className="p-4 rounded-xl border border-border bg-card space-y-2">
                  ...
                  <TableRow key={c.id} className="cursor-pointer" onClick={() => setDetailCandidate(c)}>
                    ...
                    <TableCell className="hidden lg:table-cell">
                      <div className="text-sm">
                        {c.current_ctc !== undefined && c.current_ctc !== null ? (
                          <span className="text-muted-foreground">₹{c.current_ctc}L</span>
                        ) : null}
                        {c.current_ctc !== undefined && c.current_ctc !== null && c.expected_ctc !== undefined && c.expected_ctc !== null ? (
                          <span className="text-muted-foreground"> → </span>
                        ) : null}
                        {c.expected_ctc !== undefined && c.expected_ctc !== null ? (
                          <span className="font-medium">₹{c.expected_ctc}L</span>
                        ) : null}
                        {c.current_ctc === undefined && c.expected_ctc === undefined ? "—" : null}
                      </div>
                    </TableCell>
                    <TableCell>
      ...
      {detailCandidate && (
        <CandidateDetailDialog
          ...
          onEdit={() => {
            setEditCandidate(detailCandidate);
