...
interface Props {
  ...
  row: PipelineFollowupRow | null;
  /** pending follow-up id being completed (optional) */
  completeId?: string | null;
  onLogged?: () => void;
}

export function LogPipelineFollowupDialog({ open, onOpenChange, row, completeId, onLogged }: Props) {
  const { logFollowup } = useLogPipelineFollowup();
        ...
        <div className="space-y-4 py-1">
          <div className="rounded-xl bg-muted/50 p-3 space-y-1.5">
            ...
            {row.sales_person_name && (
              <div className="text-[10px] text-muted-foreground/70">Lead owner: {row.sales_person_name}</div>
            )}
          </div>
