...
export const VoiceVisualizer = forwardRef<HTMLDivElement, VoiceVisualizerProps>(
  function VoiceVisualizer({ state, className }, ref) {
    const config = stateConfig[state];
    const isActive = state !== 'idle';

    return (
      <div ref={ref} className={cn('flex flex-col items-center justify-center gap-6', className)}>
        {/* Visualizer orb */}
        <div className="relative flex items-center justify-center">
          ...
          <div
            className={cn(
              'absolute w-28 h-28 rounded-full blur-xl transition-all duration-700',
              isActive ? 'opacity-60 scale-100' : 'opacity-20 scale-75',
              state === 'listening' && 'bg-emerald-500/30',
