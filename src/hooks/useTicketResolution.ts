...
export interface TicketAiResolution {
  ...
  implementation_report: ImplementationReport | null;
...
export function useApproveResolution() {
  ...
  return useMutation({
    mutationFn: async ({ ticketId }: { resolutionId: string; ticketId: string }) => {
      const { data, error } = await supabase.functions.invoke("apply-lovable-resolution", {
        body: { ticket_id: ticketId },
      });
      if (error) throw error;
      return data;
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["ticket-comments"] });
      queryClient.invalidateQueries({ queryKey: ["ticket-notifications"] });
      toast.success("Resolution approved — AI implementation applied");
    },
    onError: (error: Error) => {
